...
@pytest.fixture(scope="session")
def registry_arg(provider_config: ProviderConfig) -> str:
    cfg = {
...
class ExecRunner(StackqlRunner):
    def __init__(self, binary: Path, registry_arg: str) -> None:
        self.binary = binary
...
@pytest.fixture(scope="session")
def pgwire_server(stackql_binary: Path, registry_arg: str, test_env: dict[str, str]):
