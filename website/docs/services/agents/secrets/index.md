...
<tr><td><b>Id</b></td><td><CopyableCode code="github.agents.secrets" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO github.agents.secrets (
encrypted_value,
key_id,
owner,
repo,
...
```sql
DELETE FROM github.agents.secrets
WHERE owner = '{{ owner }}' --required
AND repo = '{{ repo }}' --required
AND secret_name = '{{ secret_name }}' --required
;
