...
<tr><td><b>Id</b></td><td><CopyableCode code="github.agents.repos_for_secret" /></td></tr>
</tbody></table>
...
Lists all repositories that have been selected when the `visibility`<br />for repository access to a secret is set to `selected`.<br /><br />Authenticated users must have collaborator access to a repository to create, update, or read secrets.<br /><br />OAuth app tokens and personal access tokens (classic) need the `admin:org` scope to use this endpoint. If the repository is private, the `repo` scope is also required.
...
FROM github.agents.repos_for_secret
...
```sql
INSERT INTO github.agents.repos_for_secret (
org,
secret_name,
repository_id
)
...
```sql
REPLACE github.agents.repos_for_secret
SET 
...
```sql
DELETE FROM github.agents.repos_for_secret
WHERE org = '{{ org }}' --required
AND secret_name = '{{ secret_name }}' --required
